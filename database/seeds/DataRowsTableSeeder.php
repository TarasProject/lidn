...
class DataRowsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $blogpostDataType = DataType::where('slug', 'blogposts')->firstOrFail();
        ...
        $dataRow = $this->dataRow($imageDataType, 'name_img');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'image',
                'display_name' => 'name_img',
                'required'     => 0,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
        ...
        $dataRow = $this->dataRow($imageDataType, 'project_id');
        if (!$dataRow->exists) {
            $dataRow->fill([
        ...
        $dataRow = $this->dataRow($attributeDataType, 'name_attr');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
        ...
        $dataRow = $this->dataRow($blogpostDataType, 'id');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'number',
                'display_name' => 'ID',
        ...
        $dataRow = $this->dataRow($blogpostDataType, 'title');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => 'title',
        ...
        $dataRow = $this->dataRow($blogpostDataType, 'author');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => 'author',
        ...
        $dataRow = $this->dataRow($blogpostDataType, 'short_text');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
        ...
        $dataRow = $this->dataRow($blogpostDataType, 'full_text');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
        ...
        $dataRow = $this->dataRow($blogpostDataType, 'image');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'image',
                'display_name' => 'image',
        ...
        $dataRow = $this->dataRow($blogpostDataType, 'created_at');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'timestamp',
                'display_name' => __('voyager::seeders.data_rows.created_at'),
        ...
        $dataRow = $this->dataRow($blogpostDataType, 'updated_at');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'timestamp',
                'display_name' => __('voyager::seeders.data_rows.updated_at'),
        ...
        $dataRow = $this->dataRow($projectDataType, 'brand');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => 'brand',
        ...
        $dataRow = $this->dataRow($projectDataType, 'text');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => 'text',
        ...
        $dataRow = $this->dataRow($projectDataType, 'attr_id');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'select_dropdown',
        ...
        $dataRow = $this->dataRow($contactDataType, 'office_info');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => 'office_info',
                'required'     => 1,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
        ...
        $dataRow = $this->dataRow($contactDataType, 'office_hours');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => 'office_hours',
                'required'     => 1,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
        ...
        $dataRow = $this->dataRow($contactDataType, 'tel');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => 'tel',
